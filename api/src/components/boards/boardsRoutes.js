...
router.post(
  `${ROUTE_PREFIX}/new`,
  [validateNewBoard, checkDuplicatedBoardName],
...
router.get(
  `${ROUTE_PREFIX}/all`,
  handler(getBoards, (_, response) => [response.locals.user.id, response])
);

router.get(
  `${ROUTE_PREFIX}/favorites`,
  handler(getFavorites, (_, response) => [response.locals.user.id, response])
);

router.put(
  `${ROUTE_PREFIX}/:id/toggle-favorite`,
...
router.delete(
  `${ROUTE_PREFIX}/:id`,
  handler(deleteBoard, (request, response) => [request.params.id, response])
);

router.get(
  `${ROUTE_PREFIX}/:id`,
  handler(getBoard, (request, response) => [request.params.id, response])
);
